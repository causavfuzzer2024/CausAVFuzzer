...
from .proto_utils import get_pb_from_file
...
def is_in_junction(p_point, map):
    ...
    for id, lane_polygon in map.junctions.items():
        lane_polygon_convex = lane_polygon.convex_hull
        if lane_polygon_convex.contains(p_point):
            return True
    return False
...
def get_signal_position(signal_id, map):
    if signal_id in map.signals:
        signal_pos_plgn = map.signals[signal_id]['pos']
        return list(signal_pos_plgn.exterior.coords)
...
class Map():
    ...
    def load_map(self, map_file):
        map_pb = map_pb2.Map()
        result = get_pb_from_file(map_file, map_pb)
        return result
    ...
    def parse(self):
        ...
        # check and record_example the overlap lanes with the stop sign
