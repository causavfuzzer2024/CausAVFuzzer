...
def recorder(recording_name):
    # print(recording_name)  # DEBUG
    os.system("sh recorder.sh {}".format(recording_name))


def stop_recorder(): 
    os.system("sh stop_recorder.sh")


class Server:
    def __init__(self):
        self.SIMULATOR_HOST = os.environ.get("SIMULATOR_HOST", "ip_of_your_simulator")  # ATTENTION: Please modify it to your IP address
        self.SIMULATOR_PORT = int(os.environ.get("SIMULATOR_PORT", 8181))
        self.BRIDGE_HOST = os.environ.get("BRIDGE_HOST", "ip_of_your_bridge")  # ATTENTION: Please modify it to your IP address
    ...
    def process_with_data_lgsvl_part(self, data_to_process:dict):
        ...
        try:
            ...
            self.ego.connect_bridge(os.environ.get("BRIDGE_HOST", "ip_of_your_bridge") ,port = 9090)  # Attention: Please modify it to your IP address
        ...
        if msg['CMD'] == 'CMD_READY_FOR_NEW_TEST':
            ...
            if self.result =={}:
                ...
                if self.exception:
                    ...
                    # Start cyber_recorder inside Apollo's docker
                    process = multiprocessing.Process(target=recorder, args=(recording_name, ))
                    process.start()
                    time.sleep(1)
                    process.terminate()
            else:
            ...
            if self.result.__contains__('trace'):
                ...
                self.result = {}   
                # Stop cyber_recorder inside Apollo's docker       
                process = multiprocessing.Process(target=stop_recorder)
                process.start()
                time.sleep(1)
                process.terminate()
            else:
