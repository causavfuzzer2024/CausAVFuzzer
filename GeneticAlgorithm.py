...
# For processing driving recordings
...
offset_offset = 5.0
recording_path = '<your_recording_file_directory>'  # ATTENTION: Modify it to your directory
...
class EncodedTestCase:
    def __init__(self, trace, spec, testcase_name, cav_graph, map_params_dir, vehicle_params_dir):
        ...
        # Diversity
        self.cav_graph = cav_graph
        self.map_params_dir = map_params_dir
        self.vehicle_params_dir = vehicle_params_dir
        self.testcase_name = testcase_name
        self.diversity = False  
        # self.check_diverity_update()
    ...
    def check_diverity_update(self):
        recording_file_path = recording_path + self.testcase_name + '.record'
        new_infos = processing(recording_file_path, self.map_params_dir, self.vehicle_params_dir)
        new_components = set()
        for info in new_infos: 
            flag, new_components_curr = self.cav_graph.update_graph(info)
            if flag: 
                self.diversity = flag
                # for item in new_components_curr:
                #     new_components.add(item)
            for item in new_components_curr:
                new_components.add(item)
        return new_components
    
    def get_cav_graph_updated(self): 
        return copy.deepcopy(self.cav_graph)
...
class GAGeneration:
    ...
    def mutation(self, p, lane_config, crosswalk_config):
        ...
        if True:
            ...
            new_p.chromosome[2] = list(np.random.uniform(0, 0.5, len(new_p.chromosome[2])))  # mild weather
            # mutate vehicle type
            if no_npc:
    ...
    def mutation_npc_route(self, p, lane_config, crosswalk_config):
        pass  # TODO: mutate the npc's route 
...
class CAVGAGeneration: 
    def __init__(self, population_spec, population_dive, crossover_prob=1.0, mutation_prob=1.0):
        self.population_spec = copy.deepcopy(population_spec) # a list of EncodedTestCase
        self.population_dive = copy.deepcopy(population_dive) # a list of EncodedTestCase
        self.p_cross = crossover_prob
        self.p_mutation = mutation_prob
        self.population_spec_size = len(self.population_spec)
        self.population_dive_size = len(self.population_dive)

    def selection_spec(self, pop_size_half):
        selected_population = []
        sorted_pop = sorted(self.population_spec, key=operator.attrgetter('fitness'), reverse=True)
        for i in range(pop_size_half):
            first_int = random.sample(range(0, math.ceil(self.population_spec_size/2)), 1)[0]
            second_int = random.sample(range(0, self.population_spec_size), 1)[0]
            p1 = copy.deepcopy(sorted_pop[first_int])
            p2 = copy.deepcopy(sorted_pop[second_int])
            if p1.fitness > p2.fitness:
                selected_population.append(p1)
    ...
    def selection_dive(self, pop_size_half):
        selected_population = []
        sorted_pop = sorted(self.population_dive, key=operator.attrgetter('fitness'), reverse=True)
        for i in range(pop_size_half):
            first_int = random.sample(range(0, math.ceil(self.population_dive_size/2)), 1)[0]
            second_int = random.sample(range(0, self.population_dive_size), 1)[0]
            p1 = copy.deepcopy(sorted_pop[first_int])
            p2 = copy.deepcopy(sorted_pop[second_int])
            if p1.fitness > p2.fitness:
                selected_population.append(p1)
    ...
    def one_generation_cav(self, population_size):
        # always keep the individual with the minimal fitness
        map_name = self.population_spec[0].testcase['map']
        map_info = get_map_info(map_name)
        lane_info = map_info.get_lane_config()
        crosswalk_info = map_info.get_crosswalk_config()
        # For specifications & diversity
        sorted_pop_spec = sorted(self.population_spec, key=operator.attrgetter('fitness'), reverse=True)
        sorted_pop_dive = sorted(self.population_dive, key=operator.attrgetter('fitness'), reverse=True)
        try:
            _top1_pop_spec = copy.deepcopy(sorted_pop_spec[-1])
            _top1_pop_dive = copy.deepcopy(sorted_pop_dive[-1])
        except IndexError:
            print(len(sorted_pop_spec), len(sorted_pop_dive))
        _new_population = [_top1_pop_spec, _top1_pop_dive]
        # Selecting candidates
        # selected_pop = self.selection2(population_size-1)  # only need to select n-1 individual and perform mutation.
        selected_pop_spec = self.selection_spec(int(population_size/2) - 1)
        selected_pop_dive = self.selection_dive(int(population_size/2) - 1)
        for i in range(0, int(population_size/2) - 1):  # population_size/2 - 1 = 20/2 - 1 = 9
            p1 = copy.deepcopy(selected_pop_spec[i])
            p2 = copy.deepcopy(selected_pop_dive[i])
            if random.random() < self.p_cross: 
                p1, p2 = self.crossover(p1, p2)
            if random.random() < self.p_mutation:
                p1 = self.mutation(p1, lane_info, crosswalk_info)
